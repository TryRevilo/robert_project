...
if (!empty($vars['requests']) && is_array($vars['requests'])) {
	foreach ($vars['requests'] as $user) {
		...
		$url = 'action/reservations/cancel_occupied_reservation?user_guid=' . $user->guid . '&group_guid=' . $vars['entity']->guid;
		$reject_button = elgg_view('output/url', array(
			'href' => $url,
			'confirm' => elgg_echo('groups:joinrequest:remove:check'),
			'text' => elgg_echo('Cancel'),
			'class' => 'btn btn-danger margin-left-requests',
			));
		...
		<div class="reservation-requests-wrapper">
			<div class="container-fluid">
				<div class="row">
					...
					<div class="col-md-1">$reject_button</div>
				</div>
			</div>
		</div>
HTML;
	...
	echo '<p class="no-check-reserves">' . 'The room has been unlisted' . '</p>';
}